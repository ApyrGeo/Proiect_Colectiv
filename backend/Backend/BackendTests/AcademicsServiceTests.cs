...
public class AcademicsServiceTests
{
     ...
     public async Task GetUserEnrollmentValid(int userId, int subGroupId)
    {
        ...
        var enrollmentDto = new EnrollmentResponseDTO { Id = 1, UserId = user.Id, SubGroupId = subGroup.Id, User = userResponseDto, SubGroup = subGroupDto };

        _mockUserRepository.Setup(r => r.GetByIdAsync(userId)).ReturnsAsync(user);
        _mockRepository.Setup(r => r.GetEnrollmentsByUserId(userId)).ReturnsAsync(enrollments);

        _mockMapper.Setup(m => m.Map<List<EnrollmentResponseDTO>>(enrollments)).Returns(new List<EnrollmentResponseDTO> { enrollmentDto });
